        ...
        <div className="flex gap-2">
          ...
          {(job.status === "SCHEDULED" || job.status === "IN_PROGRESS") && (
            <button
              ...
              disabled={actionLoading}
              className="flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 text-sm font-medium rounded-lg transition-colors disabled:opacity-50"
            >
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">{job.scheduledDate}</p>
                  {job.scheduledTime && <p className="text-xs text-gray-400">{job.scheduledTime}</p>}
                </div>
              </div>
              <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                {job.charges.map((c) => (
                  <div key={c.id} className="flex items-center justify-between py-2">
                    ...
                    <span className="text-sm font-medium text-gray-900">${c.amount.toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {result ? (
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-900">${result.grossRevenue.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">House Cut ({job.houseCutPercent}%)</span>
                  <span className="font-medium text-red-600">-${result.houseCutAmount.toFixed(2)}</span>
                </div>
                <div className="border-t border-gray-100 pt-3 flex justify-between text-sm">
                  <span className="font-medium text-gray-700">Net Revenue</span>
                  <span className="font-semibold text-gray-900">${result.netRevenue.toFixed(2)}</span>
                </div>
                ...
                <div className="border-t border-gray-100 pt-3 space-y-2">
                  ...
                  {result.buckets.map((b) => (
                    <div key={b.name} className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900">${b.amount.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="border-t border-gray-100 pt-3 space-y-2">
                    ...
                    {result.workerPayments.map((w) => (
                      <div key={w.userId} className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold text-green-700">${w.totalPay.toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-900">${job.totalFee.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
