...
export default function WorkerPayPeriodDetailScreen() {
  const { periodId } = useLocalSearchParams<{ periodId: string }>();
  const router = useRouter();
  const [refreshing, setRefreshing] = useState(false);
  const payQuery = useWorkerPay(periodId);
  ...
  if (payQuery.isError) {
    return (
      <View style={styles.centerBox}>
        <Text style={styles.errorTitle}>Couldn't load this pay period</Text>
        <Text style={styles.errorMessage}>Check your connection and try again.</Text>
        <View style={styles.errorActions}>
          <Button variant="primary" size="md" onPress={() => payQuery.refetch()} loading={payQuery.isRefetching}>
            Retry
          </Button>
  ...
  return (
    ...
    >
      ...
      {data.ytd && (
        <Card style={styles.summaryCard}>
          <Text style={styles.sectionTitle}>Year to Date ({data.ytd.year})</Text>
          <View style={styles.statsRow}>
            <View style={styles.stat}>
              <Text style={styles.statValue}>{data.ytd.totalJobs}</Text>
              <Text style={styles.statLabel}>Jobs</Text>
            </View>
            <View style={styles.stat}>
              <Text style={styles.statValueLarge}>${data.ytd.totalEarned.toFixed(2)}</Text>
